...
def round_scores(student_scores: list[float | int]) -> list[int]:
    ...
    """
    return [int(round(score, 0)) for score in student_scores]
...
def letter_grades(highest: int) -> list[int]:
    ...
    :return: list - of lower threshold scores for each D-A letter grade interval.
            ...
            86 <= "A" <= 100
    """
    grade_steps = (highest - 40) / 4  # failing score
    return [int(highest - ((idx + 1) * grade_steps) + 1) for idx in range(4)][::-1]
...
def student_ranking(student_scores: list[int], student_names: list[str]) -> list[str]:
    ...
    return [
        f"{idx}. {name}: {score}"
        for idx, (name, score) in enumerate(zip(student_names, student_scores), 1)
    ]
...
def perfect_score(student_info: list[list[str | int]]) -> list[str | int]:
    ...
    :return: list - first `[<student name>, 100]` or `[]`.
    """
    perfects = [[name, score] for name, score in student_info if score == 100]
    return perfects[0] if perfects else []
